...
%   Dimitra Lappa, 2015-12-10
    ...
    elseif strcmpi(choice, '1')
        model.metInChIString{i,1} = '';
        model.metChEBIID{i,1} = '';
    elseif strcmpi(choice, '2')
        choiceParts = strsplit(choice, ', ');
        newChoice = str2num(choiceParts{1,2});
        model.metInChIString{i,1} = stored_inchi_2{newChoice, 1};
        model.metChEBIID{i,1} = stored_chebi_2{newChoice, 1};
        fprintf('Proper Storing of Identifiers \n');
    elseif strfind(choice, '3')
        choiceParts = strsplit(choice, ', ');
        newChoice = str2num(choiceParts{1,2});
        model.metInChIString  {i,1} = stored_inchi_3{newChoice, 1};
        model.metChEBIID{i,1} = stored_chebi_3{newChoice, 1};
        fprintf('Proper Storing of Identifiers \n');
    else
